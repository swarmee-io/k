...
const (
	IO_SWARMEE_IPV4            = "io.swarmee.ipv4."
	IO_SWARMEE_NETWORK_ALIASES = "io.swarmee.network-aliases."
)

func (c *containerAdapter) create(ctx context.Context) error {
	var cr containertypes.ContainerCreateCreatedBody
	var err error
	ccc := types.ContainerCreateConfig{
	...
	// detect io.swarmee.privileged
	if val, ok := ccc.Config.Labels["io.swarmee.privileged"]; ok {
		v := strings.ToLower(val)
		if v == "true" || v == "1" {
			ccc.HostConfig.Privileged = true
		}
	}

	// detect io.swarmee.cap_add
	if val, ok := ccc.Config.Labels["io.swarmee.cap_add"]; ok {
		ccc.HostConfig.CapAdd = strings.Split(val, ",")
	}

	// detect io.swarmee.cap_drop
	if val, ok := ccc.Config.Labels["io.swarmee.cap_drop"]; ok {
		ccc.HostConfig.CapDrop = strings.Split(val, ",")
	}

	// detect "io.swarmee.ipv4.<net>=10.0.0.1", assign it
	// detect "io.swarmee.network-aliases.<net>=abc,def", assign it
	for k, v := range ccc.Config.Labels {
		if strings.HasPrefix(k, IO_SWARMEE_IPV4) {
			logrus.Debugf("SwarmEE IPv4 key = %v", k)
			logrus.Debugf("SwarmEE IPv4 val = %v", v)
			netName := strings.Replace(k, IO_SWARMEE_IPV4, "", 1)
			logrus.Debugf("SwarmEE Setting an IPv4 for Network = %v", netName)
			if epc, ok := ccc.NetworkingConfig.EndpointsConfig[netName]; ok {
				logrus.Debugf("SwarmEE Found Network = %v", netName)
				if epc.IPAMConfig == nil {
					epc.IPAMConfig = &network.EndpointIPAMConfig{}
				}
				epc.IPAMConfig.IPv4Address = v
				logrus.Debugf("SwarmEE IPv4 is successfully set.")
			}
		}

		if strings.HasPrefix(k, IO_SWARMEE_NETWORK_ALIASES) {
			netName := strings.Replace(k, IO_SWARMEE_NETWORK_ALIASES, "", 1)
			if epc, ok := ccc.NetworkingConfig.EndpointsConfig[netName]; ok {
				aliases := strings.Split(v, ",")
				epc.Aliases = append(epc.Aliases, aliases...)
			}
		}
	}

	if val, ok := ccc.Config.Labels["io.swarmee.pid"]; ok {
		ccc.HostConfig.PidMode = containertypes.PidMode(val)
	}

	if val, ok := ccc.Config.Labels["io.swarmee.ipc"]; ok {
		ccc.HostConfig.IpcMode = containertypes.IpcMode(val)
	}

	if val, ok := ccc.Config.Labels["io.swarmee.network_mode"]; ok {
		logrus.Debugf("SwarmEE Network Mode Enabled")
		parts := strings.SplitN(val, ":", 2)
		if len(parts) == 2 {
			if parts[0] == "service" {
				logrus.Debugf("SwarmEE Network Mode - service: detected")
				// find container
				d := c.backend.(*daemon.Daemon)
				ft := filters.NewArgs()
				ft.Add("label","com.docker.swarm.service.name=" + parts[1])
				ctrs, err := d.Containers(&types.ContainerListOptions{
					Filters: ft,
				})
				if err != nil {
					return fmt.Errorf("Cannot listing containers to find a service to attach")
				}
				if len(ctrs) == 0 {
					return fmt.Errorf("Cannot find a service to attach the network")
				}
				if len(ctrs) > 1 {
					return fmt.Errorf("Service tasks available more than 1, abort network attaching")
				}
				ccc.HostConfig.NetworkMode = containertypes.NetworkMode("container:" + ctrs[0].Names[0])
				ccc.NetworkingConfig = nil
				logrus.Debugf("SwarmEE Configured ccc.HostConfig.NetworkMode=%v", ccc.HostConfig.NetworkMode)
			}
		} else {
			ccc.HostConfig.NetworkMode = containertypes.NetworkMode(val)
			ccc.NetworkingConfig = nil
			logrus.Debugf("SwarmEE Configured ccc.HostConfig.NetworkMode=%v", ccc.HostConfig.NetworkMode)
		}
	}

	if cr, err = c.backend.CreateManagedContainer(ccc); err != nil {
		return err
	}

	if _, ok := c.container.labels()["io.swarmee.network_mode"]; ok == false {
...
func (c *containerAdapter) start(ctx context.Context) error {
	...
	err := c.backend.ContainerStart(c.container.name(), nil, "", "")
	...
	// detect "io.swarmee.init-containers=xxx,yyy"
	// detect "io.swarmee.init-container.xxx=<base64>"
	if val, ok := labels["io.swarmee.init-containers"]; ok {
		nameOrID := c.container.nameOrID()
		containerRef := "container:" + nameOrID
		names := strings.Split(val, ",")
		for _, name := range names {
			b64 := labels["io.swarmee.init-container." + name]
			jsonBytes, err := base64.StdEncoding.DecodeString(b64)
			if err != nil {
				return fmt.Errorf("Cannot decode init-container %s from base64", name)
			}
			args := []string{}
			err = json.Unmarshal(jsonBytes, &args)
			if err != nil {
				return fmt.Errorf("Cannot unmarshal init-container %s from json", name)
			}
			args = append([]string{
				"run",
				...
				"--pid", containerRef,
				"--ipc", containerRef,
				"--volumes-from", nameOrID,
			}, args...)

			cmd := osexec.Command("docker", args...)
			output, err := cmd.CombinedOutput()
			if err != nil {
				logrus.Errorf("Failed to start: docker %s", strings.Join(args," "))
				logrus.Error(string(output))
				return fmt.Errorf("Cannot init %s with %s: %v", nameOrID, name, err)
			}
		}
	}
...
func (c *containerAdapter) activateServiceBinding() error {
	// if network mode is container, not activate service binding
	if _, ok := c.container.labels()["io.swarmee.network_mode"]; ok {
		return nil
	}
